...
public class HoaDonNhapSachDao extends LibrarianDAO<HoaDonNhapSach, Long> {
    ...
    protected List<HoaDonNhapSach> selectBySql(String sql, Object... args) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                hdns.setNguoiXuLy(quanLyDao.selectByID(rs.getString("MaQL")));
                hdns.setTongTien(rs.getDouble("TongTien"));
                hdns.setNgayTao(rs.getDate("NgayThem"));
                hdns.setQr_code(rs.getString("QR_FILE"));
                hdns.setListHoaDonNhapSachChiTiets(hoaDonNhapSachChiTietDao.selectALLByHoaDon(hdns.getId()));
                list.add(hdns);
            }
    ...
    public static void main(String[] args) {
        HoaDonNhapSachDao.getInstance().selectALL().forEach((hd)->System.out.println(hd));
    }
}
